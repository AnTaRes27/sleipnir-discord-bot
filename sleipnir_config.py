...
class Config():
    ...
    def __init__(self, config_filename, ori_config_filename):
        with open(config_filename, 'r') as config_file, \
            ...
            shutil.copy(config_filename, config_filename + '.bak') #create backup
            config_file.seek(0)
            cfg = toml.load(config_file)
            ori_cfg = toml.load(ori_config_file)


            if cfg['sleipnir_version'] != ori_cfg['sleipnir_version']:
                config_file.seek(0)
                new_cfg = config_file.read().splitlines()
                for lineidx, line in enumerate(new_cfg):
                    if new_cfg[lineidx].find('sleipnir_version') != -1:
                        cmtidx = new_cfg[lineidx].find('#')  #preserving comments
                        if cmtidx != -1:
                            comment = new_cfg[lineidx][cmtidx:]
                        else:
                            comment = ''
                        new_cfg[lineidx] = toml.dumps({'sleipnir_version': ori_cfg['sleipnir_version']})[0:-1] + '    ' + comment
                    lineidx = lineidx + 1
                with open(config_filename, 'w') as new_config_file:
                    new_config_file.write('\n'.join(new_cfg))
                    print('// SLEIPNIR v{} *NEWLY UPDATED*'.format(ori_cfg['sleipnir_version']))
            else:
                print('// SLEIPNIR v{}'.format(cfg['sleipnir_version']))

            online_cfg = toml.loads(requests.get('https://raw.githubusercontent.com/AnTaRes27/sleipnir-discord-bot/master/default_config.toml').text)
            if ori_cfg['sleipnir_version'] != online_cfg['sleipnir_version']:
                print('// SLEIPNIR v{} is available!'.format(online_cfg['sleipnir_version']))
    ...
    def build_database(self):
        ...
        #          ADD COLUMN IF NOT EXISTS profanityfilter boolean
        #          ;'''
        # self.db_curs.execute(SQL)
